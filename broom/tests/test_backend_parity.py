"""The Rust core has to reproduce the Python reference tick for tick.

A two-language safety core is only worth trusting if the compiled path provably
behaves like the readable one. So we fly the same scenarios on both backends and
demand the trajectories agree down to floating-point noise.

Auto-skipped when the Rust cdylib hasn't been built.
"""
...
def test_rust_geofence_containment():
    """The containment guarantee has to hold just the same on the Rust core."""
