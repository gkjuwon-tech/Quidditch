...
-- same as the real vehicle. models a 6-axis IMU (accel + gyro with bias and
white noise) and a fused GNSS/RTK + barometer giving position & velocity at a
lower rate.
"""
...
class SensorSuite:
    ...
    def imu(self, state: State, accel_world: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        """-> (gyro_meas [body rad/s], accel_meas [body m/s^2, specific force]).

        specific force = (a_world - gravity) rotated into body frame; that's
    ...
    def gnss(self, state: State) -> tuple[np.ndarray, np.ndarray]:
        """-> (pos_meas, vel_meas) in world frame."""
    ...
    def mag(self, state: State) -> np.ndarray:
        """magnetic field direction in the BODY frame (unit-ish)."""
