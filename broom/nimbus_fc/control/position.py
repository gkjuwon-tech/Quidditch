...
This is the controller that makes "let go of the sticks and the broom just sits
there" true. Per-axis cascade:

    position error --P--> velocity setpoint --PID--> acceleration setpoint

The accel setpoint becomes a desired thrust vector (gravity-compensated). Its
direction sets the desired attitude (tilt to accelerate); its projection onto
the current body-up sets collective thrust. Tilt is hard-limited so the vehicle
can never commit to an unrecoverable bank.

Lifted from PX4's mc_pos_control, pared down to what the broom needs.
"""
...
class PositionController:
    ...
    def update(self, sp: Setpoint, state: State, dt: float):
        """-> (collective_thrust_N, q_des, yaw_rate_ff)."""
        p = self.p
        pos, vel = state.pos, state.vel

        # horizontal: pos -> vel_sp -> accel_sp.
        # stopping-distance profile so it can always brake within its accel
        # budget (no overshoot-into-a-flip). position hold is per-axis: a NaN
        # component means velocity mode on that axis (hold lateral while
        # climbing, etc).
    ...
    def _attitude_from_thrust(self, thrust_vec: np.ndarray, yaw: float) -> np.ndarray:
        """build body->world rotation: body-z along thrust, given yaw."""
