"""3D math: vectors, quaternions, rotations. numpy only, no scipy.

Conventions used everywhere in the stack (don't change these in one place):

  world frame  : ENU, x=East y=North z=Up, gravity = -z
  quaternion   : q = [w,x,y,z], unit norm, rotates body -> world
                  v_world = q (x) [0,v_body] (x) q^-1
  body frame   : x=forward y=left z=up, thrust acts along +body z
"""
...
def clamp_norm(v: np.ndarray, max_norm: float) -> np.ndarray:
    """Scale v down so |v| <= max_norm, keep the direction."""
    n = float(np.linalg.norm(v))
...
def quat_normalize(q: np.ndarray) -> np.ndarray:
    ...
    q = q / n
    # canonical hemisphere (w >= 0) so the sign doesn't flip around on us
...
def quat_rotate(q: np.ndarray, v: np.ndarray) -> np.ndarray:
    """body vector -> world: q (x) [0,v] (x) q^-1."""
...
def quat_to_rotmat(q: np.ndarray) -> np.ndarray:
    """body->world R; columns are the body axes expressed in world."""
    w, x, y, z = q
    return np.array([
...
def rotmat_to_quat(R: np.ndarray) -> np.ndarray:
    # Shepperd's method -- branchless for the largest-diagonal case to avoid
    # dividing by something tiny near the gimbal edges.
...
def quat_from_euler(roll: float, pitch: float, yaw: float) -> np.ndarray:
    """ZYX intrinsic (yaw, pitch, roll). Angles in radians."""
...
def quat_to_euler(q: np.ndarray) -> np.ndarray:
    """[roll, pitch, yaw] (rad), ZYX. pitch is clamped to dodge the singularity."""
    w, x, y, z = q
    sinr_cosp = 2 * (w * x + y * z)
...
def quat_integrate(q: np.ndarray, omega_body: np.ndarray, dt: float) -> np.ndarray:
    """first-order attitude step from body rate: q_dot = 0.5 q (x) [0,w]."""
...
def quat_error_angle_axis(q_cur: np.ndarray, q_des: np.ndarray) -> np.ndarray:
    ...
    q_err = q_cur^-1 (x) q_des. This is the PX4 attitude-error trick that
    feeds the rate setpoint.
    """
    q_err = quat_mul(quat_conj(q_cur), q_des)
    if q_err[0] < 0.0:
        q_err = -q_err  # shortest way round
    return 2.0 * q_err[1:]
...
def quat_from_rotvec(rv: np.ndarray) -> np.ndarray:
    # exp map: rotation vector (axis*angle) -> unit quat
