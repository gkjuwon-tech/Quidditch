"""Parameter store for the NIMBUS broom eVTOL.

One dataclass, grouped by subsystem. Every magic number in the stack lives here
so tuning doesn't mean hunting through modules. SI units throughout
(m, kg, s, N, Nm, rad).
"""
...
def _default_fan_layout() -> np.ndarray:
    ...
    Columns: [x, y, z, spin], x=forward y=left z=up (body). Lateral y-offset
    buys roll authority, x-spread buys pitch authority, alternating spin buys
    yaw authority. 4x8 allocation is full rank.

    The tail (bristles) flares a bit wider -- like a real broom.
    """
    return np.array([
        [+0.90, +0.22, -0.05, +1.0],
        [+0.90, -0.22, -0.05, -1.0],
...
class Params:
    ...
    pos_loop_div: int = 4           # position loop runs at 400/4 = 100 Hz

    # mass / inertia  (rider ~80 kg + vehicle ~40 kg). long thin body: low
    # roll inertia, high pitch/yaw inertia.
    mass: float = 120.0
    ...
    fan_thrust_min: float = 0.0     # fans are single-direction, can't pull
    ...
    # rider envelope (fly-by-intent limits) -- this is the "you can't crash" bit
    ...
    kp_pos_xy: float = 0.7
    kp_pos_z: float = 1.3
    # velocity-loop P sits below the tilt bandwidth, strong D damping -- this is
    # what kills the attitude limit cycle on a heavy tilt-to-translate vehicle.
    # tuned in SITL.
    kp_vel_xy: float = 2.0
    ...
    ki_vel_z: float = 3.0
    kd_vel_z: float = 0.2
    vel_i_limit: float = 6.0        # m/s^2 anti-windup clamp on vel I-term
    max_accel_xy: float = 5.0       # m/s^2 commanded accel clamp (~27 deg tilt)

    # attitude controller (P on quat error -> rate setpoint).
    # gains kept gentle: 120 kg with ~60 kg.m^2 pitch/yaw inertia can't honor
    # aggressive rate demands without saturating the fans.
    ...
    # rate controller (PID -> body torque). sized to stay inside the fan
    # cluster's torque capacity (~860 Nm pitch/yaw) on normal demands.
    ...
    # geofence (pitch keep-in volume + boundary handling)
    fence_ceiling: float = 150.0    # m, soft ceiling -- vertical Quidditch is real
                                    # (altitude_ceiling.py: thrust margin holds to
                                    #  km; chute survivable from here)
    ...
    batt_land_soc: float = 0.15       # below this -> land, now
    # pit sits inside the keep-in box, not at the hard wall.
    ...
    def __post_init__(self) -> None:
        # callers sometimes pass lists; coerce to float ndarrays
